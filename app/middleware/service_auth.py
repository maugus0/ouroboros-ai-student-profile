...
async def require_service_token(request: Request) -> None:
    ...
    """
    claims = _decode_internal_service_token(request.headers.get("Authorization"))
    if claims is not None:
        return
    raise HTTPException(status_code=401, detail="Internal bearer token required")


def _decode_internal_service_token(authorization: str | None) -> dict | None:
    ...
    verification_key = _resolve_internal_token_verification_key(token)
...
def _fetch_jwks_keys(jwks_url: str) -> dict[str, object]:
    with httpx.Client(timeout=settings.INTERNAL_TOKEN_JWKS_TIMEOUT_SECONDS) as client:
...
def _resolve_jwks_key(jwks_url: str, kid: str) -> object | None:
    now = time.time()
    ...
    try:
        fetched_keys = _fetch_jwks_keys(jwks_url)
    except Exception as exc:  # pylint: disable=broad-exception-caught
...
def _resolve_internal_token_verification_key(token: str) -> object | str | None:
    try:
    ...
    if kid and settings.INTERNAL_TOKEN_JWKS_URL:
        jwks_key = _resolve_jwks_key(settings.INTERNAL_TOKEN_JWKS_URL, str(kid))
...
def _get_user_id_from_authorization_header(authorization: str | None) -> str | None:
    claims = _decode_internal_service_token(authorization)
    if claims is None:
        return None
...
async def get_request_user_id(request: Request) -> str:
    ...
    user_id_from_jwt = _get_user_id_from_authorization_header(request.headers.get("Authorization"))
    if user_id_from_jwt:
